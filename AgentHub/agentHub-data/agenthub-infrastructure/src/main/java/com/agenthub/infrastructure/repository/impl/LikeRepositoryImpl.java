...
public class LikeRepositoryImpl implements LikeRepository {
    ...
    public void incrementTargetLikeCount(Long targetId, LikeTargetType targetType) {
        if (targetType == LikeTargetType.AGENT_CARD) {
            likeMapper.incrementCardLikeCount(targetId);
        } else if (targetType == LikeTargetType.COMMENT) {
            likeMapper.incrementCommentLikeCount(targetId);
        }
    }
    ...
    public void decrementTargetLikeCount(Long targetId, LikeTargetType targetType) {
        if (targetType == LikeTargetType.AGENT_CARD) {
            likeMapper.decrementCardLikeCount(targetId);
        } else if (targetType == LikeTargetType.COMMENT) {
            likeMapper.decrementCommentLikeCount(targetId);
        }
    }

    @Override
    public List<Long> findLikedTargetIds(Long userId, String userType, List<Long> targetIds, LikeTargetType targetType) {
        if (userId == null || targetIds == null || targetIds.isEmpty()) {
            return Collections.emptyList();
        }
        return likeMapper.selectLikedTargetIds(userId, userType, targetType.getCode(), targetIds);
    }
}