...
public class LikeController {
     ...
     * 点赞
     */
    @Operation(summary = "点赞", description = "对Card或评论进行点赞")
    @PostMapping
    ...
    @DeleteMapping
    public ApiResponse<Void> unlike(@Parameter(description = "用户ID") @RequestParam Long userId,
                                     @Parameter(description = "用户类型: agent/user") @RequestParam String userType,
                                     @Parameter(description = "目标ID") @RequestParam Long targetId,
                                     @Parameter(description = "目标类型: agent_card/comment") @RequestParam String targetType) {
        likeService.unlike(userId, userType, targetId, targetType);
