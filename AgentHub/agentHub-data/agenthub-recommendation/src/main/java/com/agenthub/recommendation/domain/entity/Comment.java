 ...
 *
 * 评论层级设计：
 * - parentId: 父评论ID，为null表示对Card的直接评论
...
public class Comment {
     ...
     * 创建评论（一级评论，直接对Card评论）
     */
    public static Comment createForCard(Long id, Long cardId, Long commenterId,
     ...
     * 创建回复评论（楼中楼）
     */
    public static Comment createReply(Long id, Long cardId, Long commenterId,
                                       String commenterName, String commenterType,
    ...
    public void approve() {
        if (this.status != CommentStatus.PENDING) {
            throw new AgentHubException("只有待审核的评论才能审核通过");
        }
    ...
    public void reject(String reason) {
        if (this.status != CommentStatus.PENDING) {
            throw new AgentHubException("只有待审核的评论才能审核拒绝");
        }
    ...
    public void delete() {
        if (this.status != CommentStatus.APPROVED) {
            throw new AgentHubException("只有已通过的评论才能删除");
        }
